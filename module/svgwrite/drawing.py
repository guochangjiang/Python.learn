...
#修改说明：2015-12-30 (guochangjiang)
#1. 修正渐变色(删除currentcolor)使得兼容coreldraw与Illustrator等；
#2. 为内容增加换行与合适的缩进
#3. viewbox会导致coreldraw崩溃，暂未修改
...
class Drawing(SVG, ElementFactory):
    ...
    def write(self, fileobj):
        ...
        for stylesheet in self._stylesheets:
            ...
            fileobj.write("\n")
            print(stylesheet)
        svgout = self.tostring()
        svgout = svgout.replace("><",">\n<")
        svgout = svgout.replace(" currentColor","")
        svglines = svgout.splitlines()
        flag = 0
        i = 0
        while i < len(svglines):
            if re.search("^<", svglines[i]) and re.search("/>", svglines[i]):
            ...
            if svglines[i] == "</defs>":
                svglines[i] = "  " + svglines[i] + "\n"
            if re.search("^\s*<g", svglines[i]):
                svglines[i] = "\n" + svglines[i]
            if re.search("</g>$", svglines[i]):
                svglines[i] = svglines[i] + "\n"
            i = i + 1
