...
class ABIFReader:
    def __init__(self, fn):
        ...
        self.type = self.readNextString(4) #文件类型，前四个字符
        print(self.type)
        if self.type != 'ABIF':
            self.close()
    ...
    def readNextChar(self):
        return self.primUnpack('c', 1).decode()
    ...
    def readNextString(self, size): #读取指定长度字符
...
if __name__ == "__main__":
    reader = ABIFReader("test.ab1") # creates an instance of ABIFReader
    print(reader.filename)
    print("ab1 version:", reader.version)
    reader.showEntries()
    for entry in reader.entries:
        print(entry)
        print(reader.getData(entry.name, entry.number))
    #data = reader.getData()
    #print(data)
    reader.close()