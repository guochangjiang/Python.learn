...
#use MegaNGKaKs.pm;
$|++;
#####update:按指定位点顺序分别进行计算

my $glob_key=$ARGV[1];
my $loci_order=$ARGV[3];
my $out_key=$ARGV[5];
...
##################################################################################
open OUT ,">$out_key.Result_kaks.csv";
open KA,">$out_key.Result_ka.csv";
open KS,">$out_key.Result_ks.csv";
open AVE, ">$out_key.average.kaks.csv";
