...
def Gff2Simple(info, intronbase):
    simpledata=[]
    AllExonDic={}
    GeneList = []
    for line in info:
...
try:
    ...
    print(u"无法载入模块argparse，可使用pip install argparse安装")
...
parser.add_argument(
        "-intronref", "--intronreference",
        metavar = "intron_ref",
        dest = "intronref",
        default = "exon",
        type = str,
        help = "the basis to interface the intron position, e.g. exon, CDS. [default: exon]")
...
infofile.close()
if fileform == "gff":
    data = Gff2Simple(data, args.intronref)
...
try:
    ...
    print(u"无法载入模块svgwrite，可使用pip install svgwrite安装")
painty0 = 25                             #起始y轴位置
paintx0=10                               #起始x轴位置
legendx0 = 900                           #图例x轴位置
