...
'''
        FILE: get.rice.expression.png.in.HanaDB.py
       USAGE: $ python get.rice.expression.png.in.HanaDB.py -h
 DESCRIPTION: 在网站Hana-DB OS(http://evolver.psc.riken.jp/seiken/OS/graphic.cgi)中查询指定水稻基因(RAP&MSU ID)列表中
              基因的表达水平详情图输出到指定目录
...
url_site = 'http://evolver.psc.riken.jp/seiken/OS/graphic.cgi'
for locus in loci_list:
    print('Searching %s in Hana-DB OS .....' % locus)
    if '.' in locus:
        locus = re.sub('\.\d+$', '', locus)
    keyword = {'gene_id': locus, 'THR' : 1000}
